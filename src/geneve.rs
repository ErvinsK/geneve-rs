        ...
        if buffer.len()>= MIN_GENEVE_HDR {
            ...
            let data = Header {
                ...
                options: match ((buffer[0] & 0x3f)*4) as usize {
                    0 => None,
                    i =>  if i <= (buffer.len()-MIN_GENEVE_HDR) {
                                    let mut vector :Vec<TunnelOption>= vec!();
                                    while let Some(k) = TunnelOption::unmarshal(&buffer[cursor..(((buffer[0] & 0x3f)*4)+8).into()]) {
                                        cursor+=k.advance();
                                        vector.push(k);
                                    } 
                                    Some(vector)
...
fn tunnel_options_marshal() {
    let decoded = TunnelOption { option_class:0xffff, option_type:0x0a, c_flag:false, data:Some(vec!(0x00, 0x01)) };
...
fn tunnel_options_unmarshal () {
    let encoded:[u8;8]= [0xff, 0xff, 0x0a, 0x01, 0x00, 0x01, 0x00, 0x00];
    let decoded = TunnelOption { option_class:0xffff, option_type:0x0a, c_flag:false, data:Some(vec!(0x00, 0x01, 0x00, 0x00)) };
...
fn geneve_header_marshal() {
    let decoded = Header {
        ...
        vni: 0x00aaaaee,
        options: Some(vec!(
            TunnelOption { option_class:0xffff, option_type:0x0a, c_flag:false, data:Some(vec!(0x00, 0x01, 0x00, 0x00))},
            TunnelOption { option_class:0xffff, option_type:0x0b, c_flag:false, data:Some(vec!(0x00, 0x02, 0x00, 0x00))},
        )),
        options_len: 0,
    };
    let encoded:[u8;24] = [0x04, 0x00, 0x86, 0xdd, 0xaa, 0xaa, 0xee, 0x00, 0xff, 0xff, 0x0a, 0x01, 0x00, 0x01, 0x00, 0x00, 0xff, 0xff, 0x0b, 0x01, 0x00, 0x02, 0x00, 0x00];
    let mut buffer:Vec<u8>=vec!();
    decoded.marshal(&mut buffer);
    assert_eq!(buffer, encoded);
}
...
fn geneve_packet_zc_unmarshal() {
    let encoded_payload:[u8;30] = [0x04, 0x00, 0x86, 0xdd, 0xaa, 0xaa, 0xee, 0x00, 0xff, 0xff, 0x0a, 0x01, 0x00, 0x01, 0x00, 0x00, 0xff, 0xff, 0x0b, 0x01, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00];
...
fn geneve_packet_unmarshal() {
    ...
    let encoded:[u8; 27] = [0x04, 0x00, 0x86, 0xdd, 0xaa, 0xaa, 0xee, 0x00, 0xff, 0xff, 0x0a, 0x01, 0x00, 0x01, 0x00, 0x00, 0xff, 0xff, 0x0b, 0x01, 0x00, 0x02, 0x00, 0x00, 0xff, 0xff, 0xff];
